...
'	 Organization: 	
'	 Filename:     	getmscluster_service.vbs
...
Sub WMIObjmscluster_serviceDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjmscluster_serviceDisplay - Used to gather WMI object from Win32_mscluster_service
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjmscluster_serviceDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;creationclassname;description;installdate;name;nodehighestversion;nodelowestversion;started;startmode;state;status;systemcreationclassname;systemname"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.nodehighestversion
				Result = Result & ";" & objItem.nodelowestversion
				...
				Result = ""
			Next
	Call StackTraceBuilder("mscluster_service","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "mscluster_service"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM mscluster_service", "\root\mscluster")
	call WMIObjmscluster_serviceDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","mscluster_service Repport","mscluster_service Repport")
