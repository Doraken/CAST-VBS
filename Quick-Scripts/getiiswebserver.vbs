...
Sub GetWMIObj( byval StrComputerName, byval Wmiquery , WMIRoot  ) 
	...
	Dim objWMIService
	Set objWMIService = GetObject("winmgmts:\\" & StrComputerName & "\root\MicrosoftIISv2" ) 
...
Sub WMIObjiiswebserverDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjiiswebserverDisplay - Used to gather WMI object from Win32_iiswebserver
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjiiswebserverDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;appisolated;apppackageid;apppackagename;approot;caption;description;installdate;name;serverstate;sslcerthash;status"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.appisolated
				Result = Result & ";" & objItem.apppackageid
				Result = Result & ";" & objItem.apppackagename
				Result = Result & ";" & objItem.approot
				...
				Result = Result & ";" & objItem.serverstate
				If isNull(objItem.sslcerthash) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & strClean2(Join(objItem.sslcerthash, ","))
				End If
				Result = Result & ";" & objItem.status
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("iiswebserver","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "iiswebserver"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM iiswebserver", "\root\microsoftiisv2")
	call WMIObjiiswebserverDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","iiswebserver Repport","iiswebserver Repport")
