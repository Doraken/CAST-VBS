...
Sub WMIObjwin32_idecontrollerDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_idecontrollerDisplay - Used to gather WMI object from Win32_win32_idecontroller
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_idecontrollerDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = ""
			Next
	Call StackTraceBuilder("win32_idecontroller","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_idecontroller"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_idecontroller", "\root\cimv2")
	call WMIObjwin32_idecontrollerDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_idecontroller Repport","win32_idecontroller Repport")
