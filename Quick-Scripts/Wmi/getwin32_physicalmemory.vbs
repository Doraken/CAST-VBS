...
Sub GetWMIObj( byval StrComputerName, byval Wmiquery , WMIRoot  ) 
	...
	Dim objWMIService
	Set objWMIService = GetObject("winmgmts:\\" & StrComputerName & WMIRoot) 
...
Sub WMIObjwin32_physicalmemoryDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_physicalmemoryDisplay - Used to gather WMI object from Win32_win32_physicalmemory
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_physicalmemoryDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;capacity;caption;datawidth;devicelocator;formfactor;installdate;manufacturer;memorytype;model;name;partnumber;serialnumber;speed;status;totalwidth;typedetail;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & strClean2(objItem.capacity)
				Result = Result & ";" & strClean2(objItem.caption)
				Result = Result & ";" & strClean2(objItem.datawidth)
				Result = Result & ";" & strClean2(objItem.devicelocator)
				Result = Result & ";" & strClean2(objItem.formfactor)
				Result = Result & ";" & strClean2(objItem.installdate)
				Result = Result & ";" & strClean2(objItem.manufacturer)
				Result = Result & ";" & strClean2(objItem.memorytype)
				Result = Result & ";" & strClean2(objItem.model)
				Result = Result & ";" & strClean2(objItem.name)
				Result = Result & ";" & strClean2(objItem.partnumber)
				Result = Result & ";" & strClean2(objItem.serialnumber)
				Result = Result & ";" & strClean2(objItem.speed)
				Result = Result & ";" & strClean2(objItem.status)
				Result = Result & ";" & strClean2(objItem.totalwidth)
				Result = Result & ";" & strClean2(objItem.typedetail)
				Result = Result & ";" & strClean2(objItem.version)
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_physicalmemory","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_physicalmemory"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_physicalmemory", "\root\cimv2")
	call WMIObjwin32_physicalmemoryDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_physicalmemory Repport","win32_physicalmemory Repport")
