...
Sub WMIObjwin32_computersystemproductDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_computersystemproductDisplay - Used to gather WMI object from Win32_win32_computersystemproduct
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_computersystemproductDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;description;identifyingnumber;name;skunumber;uuid;vendor;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.skunumber
				Result = Result & ";" & objItem.uuid
				Result = Result & ";" & objItem.vendor
				Result = Result & ";" & objItem.version
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_computersystemproduct","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_computersystemproduct"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_computersystemproduct", "\root\cimv2")
	call WMIObjwin32_computersystemproductDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_computersystemproduct Repport","win32_computersystemproduct Repport")
