...
'	 Organization: 	
'	 Filename:     	getwin32_diskdrive.vbs
...
Sub WMIObjwin32_diskdriveDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_diskdriveDisplay - Used to gather WMI object from Win32_win32_diskdrive
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_diskdriveDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.bytespersector
				If isNull(objItem.capabilitydescriptions) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & strClean2(Join(objItem.capabilitydescriptions, ","))
				End If
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.compressionmethod
				Result = Result & ";" & objItem.configmanagererrorcode
				Result = Result & ";" & objItem.creationclassname
				Result = Result & ";" & objItem.defaultblocksize
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.deviceid
				Result = Result & ";" & objItem.errorcleared
				Result = Result & ";" & objItem.errordescription
				Result = Result & ";" & objItem.errormethodology
				Result = Result & ";" & objItem.firmwarerevision
				Result = Result & ";" & objItem.index
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.interfacetype
				Result = Result & ";" & objItem.lasterrorcode
				Result = Result & ";" & objItem.manufacturer
				Result = Result & ";" & objItem.medialoaded
				Result = Result & ";" & objItem.mediatype
				Result = Result & ";" & objItem.minblocksize
				Result = Result & ";" & objItem.model
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.needscleaning
				Result = Result & ";" & objItem.numberofmediasupported
				Result = Result & ";" & objItem.partitions
				Result = Result & ";" & objItem.pnpdeviceid
				If isNull(objItem.powermanagementcapabilities) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & strClean2(Join(objItem.powermanagementcapabilities, ","))
				End If
				Result = Result & ";" & objItem.powermanagementsupported
				Result = Result & ";" & objItem.scsibus
				Result = Result & ";" & objItem.scsilogicalunit
				Result = Result & ";" & objItem.scsiport
				Result = Result & ";" & objItem.scsitargetid
				Result = Result & ";" & objItem.sectorspertrack
				Result = Result & ";" & objItem.serialnumber
				Result = Result & ";" & objItem.signature
				Result = Result & ";" & objItem.size
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.statusinfo
				Result = Result & ";" & objItem.systemcreationclassname
				Result = Result & ";" & objItem.systemname
				Result = Result & ";" & objItem.totalcylinders
				Result = Result & ";" & objItem.totalheads
				Result = Result & ";" & objItem.totalsectors
				Result = Result & ";" & objItem.totaltracks
				Result = Result & ";" & objItem.trackspercylinder
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_diskdrive","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_diskdrive"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_diskdrive", "\root\cimv2")
	call WMIObjwin32_diskdriveDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_diskdrive Repport","win32_diskdrive Repport")
