...
Sub WMIObjwin32_networkadapterDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_networkadapterDisplay - Used to gather WMI object from Win32_win32_networkadapter
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_networkadapterDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.adaptertype
				Result = Result & ";" & objItem.adaptertypeid
				Result = Result & ";" & objItem.autosense
				...
				Result = Result & ";" & objItem.maxnumbercontrolled
				Result = Result & ";" & objItem.maxspeed
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.netconnectionid
				Result = Result & ";" & objItem.netconnectionstatus
				Result = Result & ";" & objItem.netenabled
				If isNull(objItem.networkaddresses) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.networkaddresses, ",")
				End If
				Result = Result & ";" & objItem.permanentaddress
				Result = Result & ";" & objItem.physicaladapter
				Result = Result & ";" & objItem.pnpdeviceid
				If isNull(objItem.powermanagementcapabilities) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.powermanagementcapabilities, ",")
				End If
				Result = Result & ";" & objItem.powermanagementsupported
				Result = Result & ";" & objItem.productname
				Result = Result & ";" & objItem.servicename
				Result = Result & ";" & objItem.speed
				...
				Result = Result & ";" & objItem.timeoflastreset
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_networkadapter","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_networkadapter"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_networkadapter", "\root\cimv2")
	call WMIObjwin32_networkadapterDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_networkadapter Repport","win32_networkadapter Repport")
