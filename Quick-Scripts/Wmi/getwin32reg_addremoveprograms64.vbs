...
Sub WMIObjwin32reg_addremoveprograms64Display( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32reg_addremoveprograms64Display - Used to gather WMI object from Win32_win32reg_addremoveprograms64
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32reg_addremoveprograms64Display","Add")
	...
	Dim Header
	Header = "ComputerName;Date;displayname;installdate;prodid;publisher;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.prodid
				Result = Result & ";" & objItem.publisher
				Result = Result & ";" & objItem.version
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32reg_addremoveprograms64","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32reg_addremoveprograms64"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32reg_addremoveprograms64", "\root\cimv2")
	call WMIObjwin32reg_addremoveprograms64Display(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32reg_addremoveprograms64 Repport","win32reg_addremoveprograms64 Repport")
