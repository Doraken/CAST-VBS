...
Sub WMIObjwin32_serviceDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_serviceDisplay - Used to gather WMI object from Win32_win32_service
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_serviceDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;acceptpause;acceptstop;caption;description;desktopinteract;displayname;errorcontrol;exitcode;installdate;name;pathname;servicespecificexitcode;servicetype;started;startmode;startname;state;status"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & strClean2(objItem.description)
				...
				Result = ""
			Next
	Call StackTraceBuilder("win32_service","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_service"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_service", "\root\cimv2")
	call WMIObjwin32_serviceDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_service Repport","win32_service Repport")
