...
Sub WMIObjwin32_computersystemDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_computersystemDisplay - Used to gather WMI object from Win32_win32_computersystem
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_computersystemDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.adminpasswordstatus
				Result = Result & ";" & objItem.automaticmanagedpagefile
				Result = Result & ";" & objItem.automaticresetbootoption
				Result = Result & ";" & objItem.automaticresetcapability
				Result = Result & ";" & objItem.bootoptiononlimit
				Result = Result & ";" & objItem.bootoptiononwatchdog
				Result = Result & ";" & objItem.bootromsupported
				Result = Result & ";" & objItem.bootupstate
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.chassisbootupstate
				Result = Result & ";" & objItem.creationclassname
				Result = Result & ";" & objItem.currenttimezone
				Result = Result & ";" & objItem.daylightineffect
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.dnshostname
				Result = Result & ";" & objItem.domain
				Result = Result & ";" & objItem.domainrole
				Result = Result & ";" & objItem.enabledaylightsavingstime
				Result = Result & ";" & objItem.frontpanelresetstatus
				Result = Result & ";" & objItem.infraredsupported
				If isNull(objItem.initialloadinfo) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & strClean2(Join(objItem.initialloadinfo, ","))
				End If
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.keyboardpasswordstatus
				Result = Result & ";" & objItem.lastloadinfo
				Result = Result & ";" & objItem.manufacturer
				Result = Result & ";" & objItem.model
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.nameformat
				Result = Result & ";" & objItem.networkservermodeenabled
				Result = Result & ";" & objItem.numberoflogicalprocessors
				Result = Result & ";" & objItem.numberofprocessors
				If isNull(objItem.oemlogobitmap) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & strClean2(Join(objItem.oemlogobitmap, ","))
				End If
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_computersystem","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_computersystem"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_computersystem", "\root\cimv2")
	call WMIObjwin32_computersystemDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_computersystem Repport","win32_computersystem Repport")
