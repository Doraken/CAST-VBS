...
'	 Organization: 	
'	 Filename:     	getwin32_logicaldisk.vbs
...
Sub WMIObjwin32_logicaldiskDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_logicaldiskDisplay - Used to gather WMI object from Win32_win32_logicaldisk
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_logicaldiskDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;availability;caption;compressed;description;deviceid;drivetype;filesystem;freespace;installdate;name;size;status;systemname;volumename;volumeserialnumber"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.volumename
				Result = Result & ";" & objItem.volumeserialnumber
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_logicaldisk","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_logicaldisk"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_logicaldisk", "\root\cimv2")
	call WMIObjwin32_logicaldiskDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_logicaldisk Repport","win32_logicaldisk Repport")
