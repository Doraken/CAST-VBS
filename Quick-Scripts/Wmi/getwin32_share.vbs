...
Sub WMIObjwin32_shareDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_shareDisplay - Used to gather WMI object from Win32_win32_share
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_shareDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;accessmask;allowmaximum;caption;description;installdate;maximumallowed;name;path;status;type"
...
Sub Main
	...
	WMICSVReportFile =  "win32_share"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_share", "\root\cimv2")
	call WMIObjwin32_shareDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_share Repport","win32_share Repport")
