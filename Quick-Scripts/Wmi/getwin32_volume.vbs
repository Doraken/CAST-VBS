...
Sub WMIObjwin32_volumeDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_volumeDisplay - Used to gather WMI object from Win32_win32_volume
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_volumeDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.access
				Result = Result & ";" & objItem.automount
				Result = Result & ";" & objItem.availability
				Result = Result & ";" & objItem.blocksize
				Result = Result & ";" & objItem.bootvolume
				Result = Result & ";" & objItem.capacity
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.compressed
				Result = Result & ";" & objItem.configmanagererrorcode
				Result = Result & ";" & objItem.configmanageruserconfig
				Result = Result & ";" & objItem.creationclassname
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.deviceid
				Result = Result & ";" & objItem.dirtybitset
				Result = Result & ";" & objItem.driveletter
				Result = Result & ";" & objItem.drivetype
				Result = Result & ";" & objItem.errorcleared
				Result = Result & ";" & objItem.errordescription
				Result = Result & ";" & objItem.errormethodology
				Result = Result & ";" & objItem.filesystem
				Result = Result & ";" & objItem.freespace
				Result = Result & ";" & objItem.indexingenabled
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.label
				Result = Result & ";" & objItem.lasterrorcode
				Result = Result & ";" & objItem.maximumfilenamelength
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.numberofblocks
				Result = Result & ";" & objItem.pagefilepresent
				...
				Result = Result & ";" & objItem.purpose
				Result = Result & ";" & objItem.quotasenabled
				Result = Result & ";" & objItem.quotasincomplete
				Result = Result & ";" & objItem.quotasrebuilding
				Result = Result & ";" & objItem.serialnumber
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.statusinfo
				Result = Result & ";" & objItem.supportsdiskquotas
				Result = Result & ";" & objItem.supportsfilebasedcompression
				Result = Result & ";" & objItem.systemcreationclassname
				Result = Result & ";" & objItem.systemname
				Result = Result & ";" & objItem.systemvolume
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_volume","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_volume"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_volume", "\root\cimv2")
	call WMIObjwin32_volumeDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_volume Repport","win32_volume Repport")
