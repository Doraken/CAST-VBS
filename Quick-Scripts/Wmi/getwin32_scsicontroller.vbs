...
Sub WMIObjwin32_scsicontrollerDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_scsicontrollerDisplay - Used to gather WMI object from Win32_win32_scsicontroller
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_scsicontrollerDisplay","Add")
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.maxdatawidth
				Result = Result & ";" & objItem.maxnumbercontrolled
				Result = Result & ";" & objItem.maxtransferrate
				...
				Result = Result & ";" & objItem.protectionmanagement
				Result = Result & ";" & objItem.protocolsupported
				...
				Result = ""
			Next
	Call StackTraceBuilder("win32_scsicontroller","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_scsicontroller"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_scsicontroller", "\root\cimv2")
	call WMIObjwin32_scsicontrollerDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_scsicontroller Repport","win32_scsicontroller Repport")
