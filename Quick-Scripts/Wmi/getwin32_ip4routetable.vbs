...
Sub WMIObjwin32_ip4routetableDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_ip4routetableDisplay - Used to gather WMI object from Win32_win32_ip4routetable
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_ip4routetableDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;age;caption;description;destination;information;installdate;interfaceindex;mask;metric1;metric2;metric3;metric4;metric5;name;nexthop;protocol;status;type"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.metric2
				Result = Result & ";" & objItem.metric3
				Result = Result & ";" & objItem.metric4
				Result = Result & ";" & objItem.metric5
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.nexthop
				Result = Result & ";" & objItem.protocol
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.type
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_ip4routetable","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_ip4routetable"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_ip4routetable", "\root\cimv2")
	call WMIObjwin32_ip4routetableDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_ip4routetable Repport","win32_ip4routetable Repport")
