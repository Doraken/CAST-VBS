...
Sub WMIObjwin32_odbcdatasourcespecificationDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_odbcdatasourcespecificationDisplay - Used to gather WMI object from Win32_win32_odbcdatasourcespecification
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_odbcdatasourcespecificationDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;checkid;checkmode;datasource;description;driverdescription;name;registration;softwareelementid;softwareelementstate;targetoperatingsystem;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.checkid
				Result = Result & ";" & objItem.checkmode
				Result = Result & ";" & objItem.datasource
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.driverdescription
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.registration
				Result = Result & ";" & objItem.softwareelementid
				Result = Result & ";" & objItem.softwareelementstate
				Result = Result & ";" & objItem.targetoperatingsystem
				Result = Result & ";" & objItem.version
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_odbcdatasourcespecification","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_odbcdatasourcespecification"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_odbcdatasourcespecification", "\root\cimv2")
	call WMIObjwin32_odbcdatasourcespecificationDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_odbcdatasourcespecification Repport","win32_odbcdatasourcespecification Repport")
