...
Sub WMIObjwin32_networkprotocolDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_networkprotocolDisplay - Used to gather WMI object from Win32_win32_networkprotocol
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_networkprotocolDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;connectionlessservice;description;guaranteesdelivery;guaranteessequencing;installdate;maximumaddresssize;maximummessagesize;messageoriented;minimumaddresssize;name;pseudostreamoriented;status;supportsbroadcasting;supportsconnectdata;supportsdisconnectdata;supportsencryption;supportsexpediteddata;supportsfragmentation;supportsgracefulclosing;supportsguaranteedbandwidth;supportsmulticasting;supportsqualityofservice"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.connectionlessservice
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.guaranteesdelivery
				Result = Result & ";" & objItem.guaranteessequencing
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.maximumaddresssize
				Result = Result & ";" & objItem.maximummessagesize
				Result = Result & ";" & objItem.messageoriented
				Result = Result & ";" & objItem.minimumaddresssize
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.pseudostreamoriented
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.supportsbroadcasting
				Result = Result & ";" & objItem.supportsconnectdata
				Result = Result & ";" & objItem.supportsdisconnectdata
				Result = Result & ";" & objItem.supportsencryption
				Result = Result & ";" & objItem.supportsexpediteddata
				Result = Result & ";" & objItem.supportsfragmentation
				Result = Result & ";" & objItem.supportsgracefulclosing
				Result = Result & ";" & objItem.supportsguaranteedbandwidth
				Result = Result & ";" & objItem.supportsmulticasting
				Result = Result & ";" & objItem.supportsqualityofservice
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_networkprotocol","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_networkprotocol"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_networkprotocol", "\root\cimv2")
	call WMIObjwin32_networkprotocolDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_networkprotocol Repport","win32_networkprotocol Repport")
