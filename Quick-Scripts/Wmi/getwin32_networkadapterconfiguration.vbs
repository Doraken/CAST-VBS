...
Sub WMIObjwin32_networkadapterconfigurationDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_networkadapterconfigurationDisplay - Used to gather WMI object from Win32_win32_networkadapterconfiguration
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_networkadapterconfigurationDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;arpalwayssourceroute;arpuseethersnap;caption;databasepath;deadgwdetectenabled;defaultipgateway;defaulttos;defaultttl;description;dhcpenabled;dhcpleaseexpires;dhcpleaseobtained;dhcpserver;dnsdomain;dnsdomainsuffixsearchorder;dnsenabledforwinsresolution;dnshostname;dnsserversearchorder;domaindnsregistrationenabled;forwardbuffermemory;fulldnsregistrationenabled;gatewaycostmetric;igmplevel;index;interfaceindex;ipaddress;ipconnectionmetric;ipenabled;ipfiltersecurityenabled;ipportsecurityenabled;ipsecpermitipprotocols,"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.arpalwayssourceroute
				Result = Result & ";" & objItem.arpuseethersnap
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.databasepath
				Result = Result & ";" & objItem.deadgwdetectenabled
				If isNull(objItem.defaultipgateway) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.defaultipgateway, ",")
				End If
				Result = Result & ";" & objItem.defaulttos
				Result = Result & ";" & objItem.defaultttl
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.dhcpenabled
				Result = Result & ";" & objItem.dhcpleaseexpires
				Result = Result & ";" & objItem.dhcpleaseobtained
				Result = Result & ";" & objItem.dhcpserver
				Result = Result & ";" & objItem.dnsdomain
				If isNull(objItem.dnsdomainsuffixsearchorder) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.dnsdomainsuffixsearchorder, ",")
				End If
				Result = Result & ";" & objItem.dnsenabledforwinsresolution
				Result = Result & ";" & objItem.dnshostname
				If isNull(objItem.dnsserversearchorder) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.dnsserversearchorder, ",")
				End If
				Result = Result & ";" & objItem.domaindnsregistrationenabled
				Result = Result & ";" & objItem.forwardbuffermemory
				Result = Result & ";" & objItem.fulldnsregistrationenabled
				If isNull(objItem.gatewaycostmetric) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.gatewaycostmetric, ",")
				End If
				Result = Result & ";" & objItem.igmplevel
				Result = Result & ";" & objItem.index
				Result = Result & ";" & objItem.interfaceindex
				If isNull(objItem.ipaddress) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.ipaddress, ",")
				End If
				Result = Result & ";" & objItem.ipconnectionmetric
				Result = Result & ";" & objItem.ipenabled
				Result = Result & ";" & objItem.ipfiltersecurityenabled
				Result = Result & ";" & objItem.ipportsecurityenabled
				If isNull(objItem.ipsecpermitipprotocols) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result &  ";"  & Join(objItem.ipsecpermitipprotocols, ",")
				End If
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_networkadapterconfiguration","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_networkadapterconfiguration"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_networkadapterconfiguration", "\root\cimv2")
	call WMIObjwin32_networkadapterconfigurationDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_networkadapterconfiguration Repport","win32_networkadapterconfiguration Repport")
