...
Sub WMIObjwin32reg_addremoveprogramsDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32reg_addremoveprogramsDisplay - Used to gather WMI object from Win32_win32reg_addremoveprograms
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32reg_addremoveprogramsDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;installdate;prodid;publisher;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = ""
			Next
	Call StackTraceBuilder("win32reg_addremoveprograms","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32reg_addremoveprograms"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32reg_addremoveprograms", "\root\cimv2")
	call WMIObjwin32reg_addremoveprogramsDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32reg_addremoveprograms Repport","win32reg_addremoveprograms Repport")
