...
Sub WMIObjwin32_systemdriverDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_systemdriverDisplay - Used to gather WMI object from Win32_win32_systemdriver
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_systemdriverDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;acceptpause;acceptstop;caption;creationclassname;description;desktopinteract;displayname;errorcontrol;exitcode;installdate;name;pathname;servicespecificexitcode;servicetype;started;startmode;startname;state;status;systemcreationclassname;systemname;tagid"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.acceptpause
				Result = Result & ";" & objItem.acceptstop
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.creationclassname
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.desktopinteract
				Result = Result & ";" & objItem.displayname
				Result = Result & ";" & objItem.errorcontrol
				Result = Result & ";" & objItem.exitcode
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.pathname
				Result = Result & ";" & objItem.servicespecificexitcode
				Result = Result & ";" & objItem.servicetype
				Result = Result & ";" & objItem.started
				Result = Result & ";" & objItem.startmode
				Result = Result & ";" & objItem.startname
				...
				Result = Result & ";" & objItem.tagid
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_systemdriver","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_systemdriver"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_systemdriver", "\root\cimv2")
	call WMIObjwin32_systemdriverDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_systemdriver Repport","win32_systemdriver Repport")
