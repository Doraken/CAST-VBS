...
Sub WMIObjwin32_bootconfigurationDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_bootconfigurationDisplay - Used to gather WMI object from Win32_win32_bootconfiguration
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_bootconfigurationDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;bootdirectory;caption;configurationpath;description;lastdrive;name;scratchdirectory;settingid;tempdirectory"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.bootdirectory
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.configurationpath
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.lastdrive
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.scratchdirectory
				Result = Result & ";" & objItem.settingid
				Result = Result & ";" & objItem.tempdirectory
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_bootconfiguration","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_bootconfiguration"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_bootconfiguration", "\root\cimv2")
	call WMIObjwin32_bootconfigurationDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_bootconfiguration Repport","win32_bootconfiguration Repport")
