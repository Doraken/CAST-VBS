...
'	 Organization: 	
'	 Filename:     	getwin32_quickfixengineering.vbs
...
Sub WMIObjwin32_quickfixengineeringDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_quickfixengineeringDisplay - Used to gather WMI object from Win32_win32_quickfixengineering
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_quickfixengineeringDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;csname;description;hotfixid;installdate;installedby;installedon"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.hotfixid
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.installedby
				Result = Result & ";" & objItem.installedon
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_quickfixengineering","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_quickfixengineering"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_quickfixengineering", "\root\cimv2")
	call WMIObjwin32_quickfixengineeringDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_quickfixengineering Repport","win32_quickfixengineering Repport")
