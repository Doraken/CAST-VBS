...
Sub WMIObjwin32_onboarddeviceDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjwin32_onboarddeviceDisplay - Used to gather WMI object from Win32_win32_onboarddevice
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjwin32_onboarddeviceDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;creationclassname;description;devicetype;enabled;hotswappable;installdate;manufacturer;model;name;otheridentifyinginfo;partnumber;poweredon;removable;replaceable;serialnumber;sku;status;tag;version"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.devicetype
				Result = Result & ";" & objItem.enabled
				Result = Result & ";" & objItem.hotswappable
				...
				Result = Result & ";" & objItem.otheridentifyinginfo
				Result = Result & ";" & objItem.partnumber
				Result = Result & ";" & objItem.poweredon
				Result = Result & ";" & objItem.removable
				Result = Result & ";" & objItem.replaceable
				Result = Result & ";" & objItem.serialnumber
				Result = Result & ";" & objItem.sku
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.tag
				Result = Result & ";" & objItem.version
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("win32_onboarddevice","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "win32_onboarddevice"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM win32_onboarddevice", "\root\cimv2")
	call WMIObjwin32_onboarddeviceDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","win32_onboarddevice Repport","win32_onboarddevice Repport")
