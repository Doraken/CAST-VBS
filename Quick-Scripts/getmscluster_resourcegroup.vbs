...
'	 Organization: 	
'	 Filename:     	getmscluster_resourcegroup.vbs
...
Sub WMIObjmscluster_resourcegroupDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjmscluster_resourcegroupDisplay - Used to gather WMI object from Win32_mscluster_resourcegroup
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjmscluster_resourcegroupDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;antiaffinityclassnames;autofailbacktype;caption;characteristics;description;failbackwindowend;failbackwindowstart;failoverperiod;failoverthreshold;flags;installdate;internalstate;name;persistentstate;state;status"
	...
	For Each objItem in WmiItemCol
				...
				If isNull(objItem.antiaffinityclassnames) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & strClean2(Join(objItem.antiaffinityclassnames, ","))
				End If
				Result = Result & ";" & objItem.autofailbacktype
				Result = Result & ";" & objItem.caption
				Result = Result & ";" & objItem.characteristics
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.failbackwindowend
				Result = Result & ";" & objItem.failbackwindowstart
				Result = Result & ";" & objItem.failoverperiod
				Result = Result & ";" & objItem.failoverthreshold
				...
				Result = ""
			Next
	Call StackTraceBuilder("mscluster_resourcegroup","REMOVE")
...
Sub Main
	...
	WMICSVReportFile =  "mscluster_resourcegroup"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM mscluster_resourcegroup", "\root\mscluster")
	call WMIObjmscluster_resourcegroupDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","mscluster_resourcegroup Repport","mscluster_resourcegroup Repport")
