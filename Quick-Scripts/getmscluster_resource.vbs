...
'	 Organization: 	
'	 Filename:     	getmscluster_resource.vbs
...
Sub GetWMIObj( byval StrComputerName, byval Wmiquery , WMIRoot  ) 
	...
	Dim objWMIService
	Set objWMIService = GetObject("winmgmts:\\" & StrComputerName & "\root\MSCluster" ) 
...
Sub WMIObjmscluster_resourceDisplay( byval WmiItemCol)
	'-------------------------------------------------------------------------------
	' WMIObjmscluster_resourceDisplay - Used to gather WMI object from Win32_mscluster_resource
	' Parameters:
	'	StrComputerName 	- IP or name  
	'	ExportMode			- export to csv HTML or js mode. ( options are : CVS , HTML , JS , FULL)
	' Returns:
	'   Execution Computername as tring.
	'-------------------------------------------------------------------------------
	Call StackTraceBuilder("WMIObjmscluster_resourceDisplay","Add")
	...
	Dim Header
	Header = "ComputerName;Date;caption;characteristics;coreresource;cryptocheckpoints;deadlocktimeout;deleterequiresallnodes;description;flags;installdate;internalstate;isalivepollinterval;localquorumcapable;looksalivepollinterval;monitorprocessid;name;pendingtimeout;persistentstate;registrycheckpoints;resourceclass;resourcespecificstatus;restartaction;restartdelay;restartperiod;restartthreshold;retryperiodonfailure;separatemonitor;state;status;subclass;type"
	...
	For Each objItem in WmiItemCol
				...
				Result = Result & ";" & objItem.characteristics
				Result = Result & ";" & objItem.coreresource
				If isNull(objItem.cryptocheckpoints) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & Join(objItem.cryptocheckpoints, ",")
				End If
				Result = Result & ";" & objItem.deadlocktimeout
				Result = Result & ";" & objItem.deleterequiresallnodes
				Result = Result & ";" & objItem.description
				Result = Result & ";" & objItem.flags
				Result = Result & ";" & DateFormatParse(objItem.installdate)
				Result = Result & ";" & objItem.internalstate
				Result = Result & ";" & objItem.isalivepollinterval
				Result = Result & ";" & objItem.localquorumcapable
				Result = Result & ";" & objItem.looksalivepollinterval
				Result = Result & ";" & objItem.monitorprocessid
				Result = Result & ";" & objItem.name
				Result = Result & ";" & objItem.pendingtimeout
				Result = Result & ";" & objItem.persistentstate
				If isNull(objItem.registrycheckpoints) Then
						Result = Result & ";" & "Unknown"
					Else
						Result = Result & ";" & strClean2(Join(objItem.registrycheckpoints, ","))
				End If
				Result = Result & ";" & objItem.resourceclass
				Result = Result & ";" & objItem.resourcespecificstatus
				Result = Result & ";" & objItem.restartaction
				Result = Result & ";" & objItem.restartdelay
				Result = Result & ";" & objItem.restartperiod
				Result = Result & ";" & objItem.restartthreshold
				Result = Result & ";" & objItem.retryperiodonfailure
				Result = Result & ";" & objItem.separatemonitor
				Result = Result & ";" & objItem.state
				Result = Result & ";" & objItem.status
				Result = Result & ";" & objItem.subclass
				Result = Result & ";" & objItem.type
				Call  WriteLine(Result)
				Result = ""
			Next
	Call StackTraceBuilder("mscluster_resource","REMOVE")
...
Function strClean3 (strtoclean)
	'-------------------------------------------------------------------------------
	' strClean3 - Used to clean string from spécials chars "[(?*"",\\<>&#~%{}+_.@:!;]+"
	' Parameters:
	'   strtoclean 	- String to clean
	' 
	' Returns:
	'   clean string.
	'-------------------------------------------------------------------------------
...
Function strClean2 (strtoclean)
	'-------------------------------------------------------------------------------
	' strClean2 - Used to clean string from spécials chars "[(?*"",\\<>&#~%{}+_.@:!;]+"
	' Parameters:
	'   strtoclean 	- String to clean
	' 
	' Returns:
...
Function strClean(strToClean)
	'-------------------------------------------------------------------------------
	' strClean  - Used to clean string from spécials chars "[(?*"",&#~%{}+.@:!;]+"
	' Parameters:
	'   strtoclean 	- String to clean
	' 
	' Returns:
	...
	For iterator = 1 to Len(strToClean)
		...
		if currentCharASCII >= 47 AND currentCharASCII <= 59 Then '0-9
			inStringArray(iterator - 1) = currentChar
		Elseif currentCharASCII >= 65 AND currentCharASCII <= 90 Then 'A-Z
			inStringArray(iterator - 1) = currentChar
		Elseif currentCharASCII >= 97 AND currentCharASCII <= 122 Then 'a-z
			inStringArray(iterator - 1) = currentChar
		Elseif currentCharASCII >= 92 AND currentCharASCII <= 92 Then 'a-z
			inStringArray(iterator - 1) = currentChar
		Elseif currentCharASCII = 32 OR currentCharASCII = 10 Then 'space or new line
			inStringArray(iterator - 1) = currentChar
		End If
...
Sub Main
	...
	WMICSVReportFile =  "mscluster_resource"
	BaseComputerName = GetComputerName(BaseComputerName)
	Call BuildLogsDirs
	Call GetWMIObj(BaseComputerName, " SELECT * FROM mscluster_resource", "\root\mscluster")
	call WMIObjmscluster_resourceDisplay(colItems)
	Call MakeTable( StrFilePathToWriteHTMLCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".HTML", StrFilePathToWriteCSVCmp & "\" & WMICSVReportFile & "-" & StrFileDate & ".csv","mscluster_resource Repport","mscluster_resource Repport")
